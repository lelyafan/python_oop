...
class BankAccount:
    ...
    def calculate_interest(self) -> float:
        """Расчет процентов по счету (общий метод для полиморфизма)"""
        if self._balance > 0:
    ...
    def process_monthly(self) -> dict:
        """
        Ежемесячная обработка счета (интерфейсный метод для полиморфизма)
        Возвращает словарь с результатами операций
        """
        result = {
